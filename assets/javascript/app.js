$(document).ready(function () {
	...
	var questionsArr = [{
		...
		},
		{
			...
			choices: ["And I thought my jokes were bad.", "Nobody panics when things go according to plan.", "Good evening ladies and gentlemen!", "Why so serious?"],
		...
		},
		{
			...
			choices: ["Why so serious?", "You're not playing the odds, friend", "It's not about money", "How 'bout a magic trick?"],
	...
	function generateAnswers(array) {
		...
		for (var i = 0; i < array[questionNum].choices.length; i++) {
			if (i !== indexCorrect) {
				var choicesDiv = $(`<div class="choice rounded">${array[questionNum].choices[i]}</div>`);
				$("#answer-container").append(choicesDiv);
			} else {
				var correctDiv = $(`<div class="choice correct rounded">${correctAnswer}</div>`);
	...
	function endGame() {
		...
		$(".main-section").html(`<h4><strong>Game Summary</strong></h4>
			...
			<p>Your Score: ${correctCount/(correctCount+incorrectCount+unansweredCount)*100}%</p>
